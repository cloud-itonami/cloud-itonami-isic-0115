(ns tobaccoops.operation
  ...
  actor run. The advisor (TobaccoOpsAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Tobacco Operations
  ...
  One run = one tobacco-growing coordination operation (intake -> advise
  -> govern -> decide -> commit | hold | approval). No unbounded inner
  loop -- each operation is auditable and checkpointed. A field's
  operating history is advanced by MANY operations (log-cultivation-record
  ...
  is clean -- see `tobaccoops.governor/always-escalate-ops`.

  NOTE: langgraph-clj StateGraph integration is deferred (mirrors
  `fibreops.operation`, cloud-itonami-isic-0116). This stub version
  ...
  (:require [tobaccoops.advisor :as advisor]
...
(defn run-operation
  "Run one tobacco-growing operation through the advisor -> governor ->
