(ns tobaccoops.phase
  "Phase gate for the tobacco-growing operations coordinator rollout
  (0->3 maturity). Mirrors `fibreops.phase` (cloud-itonami-isic-0116) in
  ...
  Phase 1: supervised operation -- always-escalate ops
    (`tobaccoops.governor/always-escalate-ops`) escalate even when clean;
  ...
  (:require [tobaccoops.governor :as governor]))
