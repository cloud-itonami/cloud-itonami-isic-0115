(ns tobaccoops.store
  "SSoT for the tobacco-growing operations coordinator, behind a `Store`
  ...
  cloud-itonami-isic-0111; `marketentry.store`, cloud-itonami-iso3166-ago):
  ...
  (test/tobaccoops/store_contract_test.cljc).
  ...
  the Governor (see `tobaccoops.governor`'s `field-registered` invariant).
  Field data is opaque to this namespace -- callers/backends decide what a
  field record contains (name, location, tobacco-type, acreage, etc);
  this Store only answers \"is this field-id registered, and if so what's
  on file\". Because the field payload shape is intentionally open,
  ...
  core missing plumbing until the deferred-stub fix: `tobaccoops.operation`'s
  `:commit`/`:hold` graph nodes append every committed/held/
  approval-rejected decision fact here, so a field's operating history
  (every `:log-cultivation-record` / `:schedule-field-operation` /
